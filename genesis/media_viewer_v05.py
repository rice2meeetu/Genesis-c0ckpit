#!/usr/bin/env python3
"""
GENESIS Photo Studio v0.5 - Auto-Tagging & Smart Search
"""
...
class FaceViewerV05(QMainWindow):
    """Main window with auto-tagging and smart search"""
    ...
    def init_ui(self):
        """Initialize UI"""
        self.setWindowTitle("GENESIS v0.5 - Auto-Tagging & Smart Search")
    ...
    def do_search(self):
        ...
        if results:
            ...
            if self.filtered_images:
                self.show_image(self.filtered_images[0][0])
            self.status.showMessage(f"Found {len(results)} photos with '{query}'")
        else:
