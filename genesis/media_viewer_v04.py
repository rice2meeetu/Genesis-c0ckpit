#!/usr/bin/env python3
"""
GENESIS Photo Studio v0.4 - Face Groups & Export
"""
...
from genesis.face_groups import FaceGroups, FaceExporter


class FaceViewerV04(QMainWindow):
    ...
    def __init__(self):
        ...
        self.groups = FaceGroups(self.db_path)
        self.exporter = FaceExporter(self.db_path)
    ...
    def create_preview(self):
        ...
        export_controls.addWidget(self.export_zip_btn)
        
        export_controls.addStretch()
        export_layout.addLayout(export_controls)
        
        # Quick export for current person
        quick_export = QHBoxLayout()
        quick_export.addWidget(QLabel("Quick Export:"))
        self.quick_person = QComboBox()
        self.update_person_list()
        quick_export.addWidget(self.quick_person)
        self.quick_export_btn = QPushButton("📤 Export")
        self.quick_export_btn.clicked.connect(self.quick_export)
        quick_export.addWidget(self.quick_export_btn)
        export_layout.addLayout(quick_export)
    ...
    def update_person_list(self):
        ...
        self.quick_person.clear()
        
        # Add "All Photos" option
        self.person_filter.addItem("All Photos")
        self.quick_person.addItem("Select person...")
        
        # Add known faces
        for name in self.detector.known_names:
            self.person_filter.addItem(name)
            self.quick_person.addItem(name)
    ...
    def filter_by_person(self, name: str):
        ...
        if name == "All Photos" or name == "---":
            ...
            # Find all photos of this person
            photos = self.groups.get_person_photos(name, limit=500)
    ...
    def export_zip(self):
        """Export album as ZIP"""
        name, ok = QInputDialog.getText(self, "Export Album", "Enter person name:")
        if ok and name.strip():
            zip_path = self.exporter.export_album_zip(name.strip())
            if zip_path:
                QMessageBox.information(self, "Success", f"✅ Exported to:\n{zip_path}")
    
    def quick_export(self):
        """Quick export selected person"""
        name = self.quick_person.currentText()
        if name and name != "Select person...":
            zip_path = self.exporter.export_album_zip(name)
            if zip_path:
                QMessageBox.information(self, "Success", f"✅ Exported to:\n{zip_path}")
